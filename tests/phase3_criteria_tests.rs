...
صدّر دالة عشوائي_بين(ادنى: عدد، اقصى: عدد) -> عدد {
    أرجع __عشوائي_صحيح__(ادنى، اقصى)
}

صدّر دالة بذرة_عشوائي(بذرة: عدد) {
    __بذرة_عشوائية__(بذرة)
}
"#;
        assert!(
            parses_ok(source),
...
صدّر دالة موجود(مسار: نص) -> منطقي {
    أرجع __مسار_موجود__(مسار)
}

صدّر دالة هو_ملف(مسار: نص) -> منطقي {
    أرجع __هو_ملف__(مسار)
}

صدّر دالة هو_مجلد(مسار: نص) -> منطقي {
    أرجع __هو_مجلد__(مسار)
}
"#;
        assert!(
            parses_ok(source),
...
صنف مجلد {
    ...
    عام دالة أنشئ() {
        __انشئ_مجلد__(هذا._مسار)
    }

    عام دالة حذف() {
        __احذف_مجلد__(هذا._مسار)
    }

    عام دالة قائمة() -> مصفوفة<نص> {
        أرجع __قائمة_مجلد__(هذا._مسار)
    }

    عام دالة موجود() -> منطقي {
        أرجع __مجلد_موجود__(هذا._مسار)
    }
}
"#;
        assert!(
...
صدّر دالة اقرأ_ملف(المسار: نص) -> نص {
    أرجع __اقرأ_ملف__(المسار)
}

صدّر دالة اكتب_ملف(المسار: نص، المحتوى: نص) {
    __اكتب_ملف__(المسار، المحتوى)
}

صدّر دالة ملف_موجود(المسار: نص) -> منطقي {
    أرجع __ملف_موجود__(المسار)
}
...
صنف طلب_شبكة {
    ...
    عام دالة احصل() -> استجابة_شبكة {
        أرجع __طلب_احضار__(هذا._رابط)
    }

    عام دالة ارسل(المحتوى: نص) -> استجابة_شبكة {
        أرجع __طلب_إرسال__(هذا._رابط، المحتوى)
    }
}
"#;
        assert!(
