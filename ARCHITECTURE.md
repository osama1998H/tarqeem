...
│
├── runtime-rs/               # مكتبة وقت التشغيل (بلغة رست)
│   ├── Cargo.toml            # إعدادات الحزمة
│   ├── src/
│   │   ├── lib.rs            # واجهة المكتبة الرئيسية
│   │   ├── types.rs          # أنماط FFI المتوافقة
│   │   ├── memory.rs         # إدارة الذاكرة وعد المراجع
│   │   ├── string.rs         # معالجة النصوص (+٤٠ دالة)
│   │   ├── array.rs          # المصفوفات الديناميكية
│   │   ├── io.rs             # الإدخال/الإخراج (+٤٠ دالة)
│   │   ├── math.rs           # الدوال الرياضية (+٤٥ دالة)
│   │   ├── runtime.rs        # تهيئة وقت التشغيل
│   │   ├── crypto.rs         # SHA256، Hex
│   │   ├── compress.rs       # ضغط GZIP
│   │   └── network.rs        # TCP، UDP، HTTP (+٢٥ دالة)
│
├── stdlib/               # المكتبة القياسية (٤٢ ملف ترقيم)
...
│       └── اختياري.ترقيم
│
├── examples/                 # أمثلة (١٠ برامج، واحد لكل مجال لغوي)
│   ├── مرحبا.ترقيم            # أول برنامج
│   ├── أساسيات.ترقيم          # المتغيرات، التحكم، المصفوفات
│   ├── دوال.ترقيم             # الدوال المُعلَنة والسهمية + القيود الحالية
│   ├── أصناف.ترقيم            # المواثيق، الرؤية، الوراثة، الخواص
│   ├── تعداد.ترقيم            # التعدادات ومطابقة الأنماط
│   ├── صياغة.ترقيم            # وضع البرنامج والأسطر المتعددة
│   ├── حاسبة.ترقيم            # برنامج حسابي كامل
│   ├── لعبة_الحياة.ترقيم      # مصفوفات ثنائية وحلقات متداخلة
│   ├── تشفير_وضغط.ترقيم       # SHA-256 و GZIP
│   └── اختبار_اطار_العمل.ترقيم # أنماط كتابة الاختبارات
│
├── tests/                    # الاختبارات
...
المكتبة القياسية مزيج من مكتبة وقت تشغيل بلغة رست وكود ترقيم:

### مكتبة وقت التشغيل (رست)

| الملف | الوظيفة |
|-------|---------|
| `memory.rs` | إدارة الذاكرة وعد المراجع |
| `string.rs` | معالجة النصوص مع دعم UTF-8 (+٤٠ دالة) |
| `array.rs` | المصفوفات الديناميكية |
| `io.rs` | الإدخال/الإخراج للملفات والمجلدات (+٤٠ دالة) |
| `math.rs` | الدوال الرياضية (+٤٥ دالة) |
| `runtime.rs` | تهيئة وقت التشغيل ومتغيرات البيئة |
| `crypto.rs` | SHA256، Hex |
| `compress.rs` | ضغط GZIP |
| `network.rs` | TCP، UDP، HTTP (+٢٥ دالة) |
