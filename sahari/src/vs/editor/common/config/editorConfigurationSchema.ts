...
const editorConfiguration: IConfigurationNode = {
	...
	properties: {
		...
		// Sahari RTL Configuration - صحاري
		'editor.autoRTL': {
			type: 'boolean',
			default: true,
			description: nls.localize('autoRTL', "Automatically detect and apply RTL (right-to-left) text direction for Arabic, Hebrew, and other RTL languages. | كشف تلقائي وتطبيق اتجاه النص من اليمين لليسار للغة العربية والعبرية.")
		},
		'sahari.editor.autoRTL': {
			type: 'boolean',
			default: true,
			description: nls.localize('sahariAutoRTL', "Enable automatic RTL detection for Arabic content in Sahari editor. | تفعيل الكشف التلقائي عن اتجاه النص للمحتوى العربي في محرر صحاري.")
		},
		'sahari.editor.arabicFontFamily': {
			type: 'string',
			default: "'Amiri', 'Noto Sans Arabic', 'Courier New', monospace",
			description: nls.localize('sahariArabicFontFamily', "Font family to use for Arabic text in the editor. | نوع الخط المستخدم للنص العربي في المحرر.")
		},
	}
};
