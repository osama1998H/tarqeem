...
/// Shared with the debug interpreter so `getcwd(2)`'s answer — the directory
/// verbatim, lossily decoded, `""` for a cwd the OS cannot report — is one
/// answer in both.
pub(crate) use executor::builtins::call_dir_current;
