...
| | |
|---|---|
| Names declared in `Scope` | **189** — 24 `core_builtins()` + 165 `get_stdlib_builtin()` across 7 modules |
| Names reachable on *some* backend | **241** — the extra 52 exist in a backend but in no registry, so no program can call them |
...
| … reachable from a declared name | **154** |
...
| Places to edit to add **one** builtin | **9** |
...
|---|---|---|
| Semantic | `src/semantic/scope.rs` | `core_builtins()` — a `Vec` of 24 `(name, params, ret)` tuples registered into the global scope. `get_stdlib_builtin(module, name)` — a two-level `match` with 165 arms, manufactured on demand at import. `get_stdlib_module_exports()` — a **second, hand-maintained copy** of the same 165 names with no consistency test. |
...
**78 of 189 declared names (41%) have no interpreter arm**, so `tarqeem run` fails on them after
...
names against the **189 declared** names; this table counts 48 against the **241 reachable** names,
