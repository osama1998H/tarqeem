...
| … of which orphans (no caller anywhere) | **24** — `trq_env_get` left the set in #338, and `trq_file_open_read`/`_write`/`_append` left it at #362, which gave all three their first non-test caller in the `trq_file_open` wrapper that folds them. `trq_file_close` left it at #364, which gave it an Arabic name — though it already had one production caller inside `trq_file_open`'s directory refusal, so the row this replaces was wrong when written |
