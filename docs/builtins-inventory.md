...
| | |
|---|---|
| Names declared in `Scope` | **187** — 22 `core_builtins()` + 165 `get_stdlib_builtin()` across 7 modules |
| Names reachable on *some* backend | **239** — the extra 52 exist in a backend but in no registry, so no program can call them |
...
|---|---|---|
| Semantic | `src/semantic/scope.rs` | `core_builtins()` — a `Vec` of 22 `(name, params, ret)` tuples registered into the global scope. `get_stdlib_builtin(module, name)` — a two-level `match` with 165 arms, manufactured on demand at import. `get_stdlib_module_exports()` — a **second, hand-maintained copy** of the same 165 names with no consistency test. |
...
**78 of 187 declared names (42%) have no interpreter arm**, so `tarqeem run` fails on them after
...
names against the **187 declared** names; this table counts 48 against the **239 reachable** names,
