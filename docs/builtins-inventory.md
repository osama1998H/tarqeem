...
|---|---|---|
| Semantic | `src/semantic/scope.rs` | `core_builtins()` — a `Vec` of 18 `(name, params, ret)` tuples registered into the global scope. `get_stdlib_builtin(module, name)` — a two-level `match` with 165 arms, manufactured on demand at import. `get_stdlib_module_exports()` — a **second, hand-maintained copy** of the same 165 names with no consistency test. |
...
**78 of 183 declared names (43%) have no interpreter arm**, so `tarqeem run` fails on them after
...
names against the **183 declared** names; this table counts 48 against the **235 reachable** names,
