...
عمليات البتات دوالٌ لا عوامل: ترقيم لا تملك رموز `&` و`|` و`^`، وإضافتها تغييرٌ في الصياغة.
وبادئة `بتات_` لأن `و` و`أو` كلمتان مفتاحيتان، فلا يصح أن يكون أحدهما اسماً بمفرده
(أما ابتداء معرِّف بهما فمسموح: `وقت`، `أولوية`).
...
و`بتات_أو_حصري` تقلب البتات بدل أن تفحصها أو ترفعها، فهي وحدها التي تتراجع بنفسها:
قلبُ القيمة بالقناع مرتين يعيدها كما كانت.
...
`عدد` صحيح ذو إشارة بـ٦٤ بتّاً، فـ`بتات_و(-1، 255)` يساوي `255`، و`بتات_أو(-1، 0)`
يساوي `-1`، و`بتات_أو_حصري(-1، -1)` يساوي `0`.
